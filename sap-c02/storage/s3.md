...
## Data protection internals (why erasure coding isn't detailed)

AWS publishes S3 durability and redundancy guarantees (e.g., "11 9's" durability, multi‑AZ replication, CRR/SRR behaviors) but does not document all low‑level storage algorithms in user guides. Implementation details such as whether erasure coding or block‑level replication are used internally can vary by storage class and are considered internal architecture choices. For exam and operational purposes, focus on the documented guarantees (replication, durability, versioning, Object Lock, replication options). For deeper implementation discussion, consult AWS whitepapers and service architecture papers or re:Invent talks.